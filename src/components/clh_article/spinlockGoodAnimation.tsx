...
const steps = [
  ...
  "Thread B observes the release and takes the lock.",
];

export default function SpinlockGoodAnimation() {
  const [step, setStep] = useState(0);

  const lockTaken = [1, 2, 4].includes(step);
  const threadAState = (lockTaken && step !== 4) ? 'active' : 'free';
  const threadAWorking = step === 1 || step === 3;
  const threadBState = step <= 1 ? 'free' : (step !== 4 ? 'waiting' : 'active');
  const threadBWorking = step === 2 || step === 4;
